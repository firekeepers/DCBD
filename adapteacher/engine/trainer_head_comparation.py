...
class ATeacherTrainer(DefaultTrainer):
    ...
    def run_step_full_semisup(self,recall_list,precise_list,mIOU_list,t_ins_list,t_list,s_ins_list,s_list,level_diction_list,source_level_diction_list,lens,lent):
        ...
        if self.iter < 10000000:
            if self.iter==0:
                self._update_student_model(keep_rate=0.00)
                self._reinitialize_student_model(keep_rate=0.00)
            ...
            all_label_target_data = unlabel_data_q + unlabel_data_k

            record_all_label_train_data, _, _, _ = self.model(
                all_label_train_data, branch="supervised_validation_DCBD"
            )
            record_dict.update(record_all_label_train_data)
            # compare_label
            record_all_label_compare_data, _, _, _ = self.model(
                all_label_target_data,branch="supervised_validation_DCBD"
            )
            new_record_all_label_compare_data = {}
            for key in record_all_label_compare_data.keys():
                new_record_all_label_compare_data[key + "_target"] = record_all_label_compare_data[
                    key
                ]
    ...
    @torch.no_grad()
    def _reinitialize_student_model(self, keep_rate=0):
        # if comm.get_world_size() > 1:
        #     student_model_dict = {
        #         key[7:]: value for key, value in self.model.state_dict().items()
        #     }
        ...
        for key, value in self.model.state_dict().items():
            ...
            if not key.startswith('roi_heads'):
                new_dict[key] = (
                        value
                )
            if key.startswith('roi_heads'):
                ...
                if 'bias' in key:
                    new_dict[key] = (nn.init.constant_(value, 0))
        self.model.load_state_dict(new_dict)

    def _reinitialize_teacher_model(self, keep_rate=0):
        # if comm.get_world_size() > 1:
        #     student_model_dict = {
        #         key[7:]: value for key, value in self.model.state_dict().items()
        #     }
        ...
        for key, value in self.model_teacher.state_dict().items():
            ...
            if key.startswith('roi_heads'):
                ...
                if 'bias' in key:
                    new_dict[key] = (nn.init.constant_(value, 0))
        self.model_teacher.load_state_dict(new_dict)
