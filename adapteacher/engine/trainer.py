...
class ATeacherTrainer(DefaultTrainer):
    ...
    def run_step_full_semisup(self,recall_list,precise_list,mIOU_list,t_ins_list,t_list,s_ins_list,s_list,level_diction_list,source_level_diction_list,lens,lent):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            # compare_label
            record_all_label_compare_data, ins_feat_t, _, _ = self.model(
        ...
        #         label_train_data_q, branch="supervised")
            ...
            losses = sum(loss_dict.values())
                ...
                # and self.iter < self.cfg.SEMISUPNET.REFINE_STEP:
            if self.iter == self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            with torch.no_grad():
                ...
                # todo:student_pred to do model_fusion
            #     (
            #         _,
            ...
            #todo: Pseudo_labeling for ROI head (bbox location/objectness)
            pesudo_proposals_roih_unsup_k, _ = self.process_pseudo_label(
...
#todo:do image-level abortion if prediction quality ,

            # ins = []
            # for i in range(len(all_unlabel_data)):
            #     num = len(all_unlabel_data[i]['instances'])
            #     ins.append(num)
            # print(ins)
            #image_shuffle
            # pseudo_data = []
            # for data in unlabel_data_q:
        ...
        # 5. input strongly augmented unlabeled data #and strongly augmented compared data# into model
            ...
            #todo:6. Scale student inputs (pseudo-labels and image),from cvpr2023-2pcnet
            ...
            #     self.stu_scale = 1.0
            # scaled_unlabel_data = [x.copy() for x in all_unlabel_data]
            ...
            # record_all_domain_data, t_ins_score, t_img_score, s_ins_score, s_img_score, level_diction, source_level_diction = self.model(
...
#todo::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::test just
            # with torch.no_grad():
            #     feats_array_s, len_s, _, _ = self.model(
            #         label_train_data_k, branch="t-SNE"
            #     )
            #     feats_array_t, len_t, _, _ = self.model(
            #         unlabel_data_k, branch="t-SNE"
            #     )
            #     for i in range(len_s):
            #         encoding_array_s.append(feats_array_s[i].squeeze().detach().cpu().numpy())
            #     for i in range(len_t):
            #         encoding_array_t.append(feats_array_t[i].squeeze().detach().cpu().numpy())
            #     lens = lens + len_s
            #     lent = lens + len_t
            ...
            for key in record_dict.keys():
                if key.startswith("loss"):
                    if key == "loss_rpn_loc_pseudo" or key == "loss_box_reg_pseudo":
                        ...
                        loss_dict[key] = record_dict[
                                             key] * self.cfg.SEMISUPNET.DIS_LOSS_WEIGHT 
