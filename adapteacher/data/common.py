...
class AspectRatioGroupedSemiSupDatasetTwoCrop(AspectRatioGroupedDataset):
    ...
    def __init__(self, dataset, batch_size):
        ...
        self.label_dataset, self.compare_dataset,self.unlabel_dataset = dataset
        self.batch_size_label = batch_size[0]
        self.batch_size_compare = batch_size[1]
        self.batch_size_unlabel = batch_size[2]

        self._label_buckets = [[] for _ in range(2)]
        self._label_buckets_key = [[] for _ in range(2)]

        self._compare_buckets = [[] for _ in range(2)]
        self._compare_buckets_key = [[] for _ in range(2)]
    ...
    def __iter__(self):
        label_bucket, compare_bucket,unlabel_bucket = [],[], []
        for d_label,d_compare, d_unlabel in zip(self.label_dataset,self.compare_dataset, self.unlabel_dataset):
            # d is a tuple with len = 2
            # It's two images (same size) from the same image instance
            ...
            if len(compare_bucket) != self.batch_size_label:
                w, h = d_compare[0]["width"], d_compare[0]["height"]
                compare_bucket_id = 0 if w > h else 1
                compare_bucket = self._compare_buckets[compare_bucket_id]
                compare_bucket.append(d_compare[0])
                compare_buckets_key = self._compare_buckets_key[compare_bucket_id]
                compare_buckets_key.append(d_compare[1])
            ...
            if (
                ...
                and len(compare_bucket) == self.batch_size_compare
            ):
