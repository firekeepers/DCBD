...
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    if cfg.DATASETS.CROSS_DATASET:  # cross-dataset (e.g., coco-additional)
        ...
        compare_dicts = get_detection_dataset_dicts(
            cfg.DATASETS.COMPARE_LABEL,
            filter_empty=False,
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
            if cfg.MODEL.KEYPOINT_ON
            else 0,
    ...
    compare_dataset = DatasetFromList(compare_dicts, copy=False)
    ...
    compare_dataset = MapDataset(compare_dataset, mapper)
    ...
    if sampler_name == "TrainingSampler":
        ...
        compare_sampler = TrainingSampler(len(compare_dataset))
    ...
    return build_semisup_batch_data_loader_two_crop(
        (label_dataset, compare_dataset,unlabel_dataset),
        (label_sampler, compare_sampler,unlabel_sampler),
        cfg.SOLVER.IMG_PER_BATCH_LABEL_TRAIN,
        cfg.SOLVER.IMG_PER_BATCH_LABEL_COMPARE,
...
def build_semisup_batch_data_loader_two_crop(
    ...
    *,
    ...
    assert (
            total_batch_size_compare > 0 and total_batch_size_compare % world_size == 0
    ), "Total label batch size ({}) must be divisible by the number of gpus ({}).".format(
        total_batch_size_compare, world_size
    )
    ...
    batch_size_compare = total_batch_size_compare // world_size
    batch_size_unlabel = total_batch_size_unlabel // world_size

    label_dataset, compare_dataset,unlabel_dataset = dataset
    label_sampler, compare_sampler,unlabel_sampler = sampler
    ...
    if aspect_ratio_grouping:
        ...
        compare_data_loader = torch.utils.data.DataLoader(
            compare_dataset,
        ...
        return AspectRatioGroupedSemiSupDatasetTwoCrop(
            (label_data_loader, compare_data_loader,unlabel_data_loader),
            (batch_size_label, batch_size_compare,batch_size_unlabel),
        )
    else:
