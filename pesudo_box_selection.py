...
def inference_on_dataset(model_s,model_p, data_loader, evaluator):
    ...
    with inference_context(model_p),inference_context(model_s), torch.no_grad():
        EMA_rate = 0.5
        # model_p = _fusion_teacher_model(model_p.modelTeacher,model_s.modelTeacher,EMA_rate)
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            outputs = WBF(output_p, output_s,iou_thr=0.6,skip_box_thr=0.0001,weights = [1,1])
            #oral
